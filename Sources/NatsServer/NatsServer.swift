...
// NatsServer spawns a real `nats-server` subprocess for integration tests and
// therefore relies on `Process`/`XCTest`, which are unavailable on iOS/tvOS/
// watchOS. Restrict the whole helper to platforms where it can actually run so
// the library still compiles when an app links the package for iOS.
    ...
    public class NatsServer {
        ...
        public func start(
            ...
            XCTAssertNil(
                self.process, "nats-server is already running on port \(port)", file: file,
                line: line)
            let process = Process()
            let pipe = Pipe()
            ...
            XCTAssertNoThrow(
                try process.run(), "error starting nats-server on port \(port)", file: file,
                line: line)

            let result = semaphore.wait(timeout: .now() + .seconds(10))

            XCTAssertFalse(
                result == .timedOut, "timeout waiting for server to be ready", file: file,
                line: line)
            XCTAssertNil(
        ...
        public func sendSignal(_ signal: Signal, file: StaticString = #file, line: UInt = #line) {
            ...
            process.executableURL = URL(fileURLWithPath: "/usr/bin/env")
            process.arguments = [
                "nats-server", "--signal", "\(signal.rawValue)=\(self.pidFile!.path)",
            ]
