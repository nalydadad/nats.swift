...
// `NWProtocolWebSocket`.
//
// This exists because `URLSessionWebSocketTask` cannot reliably carry a
// WebSocket through a network middlebox such as a tunnel-type per-app VPN:
// the upgrade (HTTP 101) succeeds but the upgraded stream is dropped before
// any frame flows, surfacing as `NSURLErrorNetworkConnectionLost` (-1005) /
// "Connection not set before response is received". `NWConnection` performs
// the TLS handshake, the WebSocket upgrade, and frame I/O on a single
// connection — there is no internal "hand the upgraded stream off" step to
// drop — and it is transparently captured by a per-app VPN's flow/packet
// interception. `NWProtocolWebSocket` provides native framing/handshake/
// control-frame handling, so there is no hand-rolled WebSocket code to
// maintain. `NWProtocolWebSocket` is available from macOS 10.15 / iOS 13,
// well below this package's deployment floor, so no availability gate is
// needed.
    ...
    internal final class NWWebSocketTransport: NatsTransport, @unchecked Sendable {
        ...
        // Bound an initial connect so a stuck handshake doesn't hang the NATS
        // connect attempt (the NATS layer retries on failure).
